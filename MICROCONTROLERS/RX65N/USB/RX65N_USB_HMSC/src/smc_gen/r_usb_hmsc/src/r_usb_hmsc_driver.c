        ...
        uint16_t side)
{
    ...
    g_usb_hmsc_cbw[ptr->ip][side].bm_cbw_flags.BIT.cbw_dir = 0;
    g_usb_hmsc_cbw[ptr->ip][side].bm_cbw_flags.BIT.reserved7 = 0;
    g_usb_hmsc_cbw[ptr->ip][side].bcbw_lun.BIT.bcbw_lun = 0; /*** Support LUN0 ONLY ***/
    g_usb_hmsc_cbw[ptr->ip][side].bcbw_lun.BIT.reserved4 = 0;
    g_usb_hmsc_cbw[ptr->ip][side].bcbwcb_length.BIT.bcbwcb_length = 0;
    g_usb_hmsc_cbw[ptr->ip][side].bcbwcb_length.BIT.reserved3 = 0;
    ...
    switch (command)
    {
        ...
        case USB_ATAPI_READ10 :
            g_usb_hmsc_cbw[ptr->ip][side].bm_cbw_flags.BIT.cbw_dir = 1;

            /* 10bytes */
            g_usb_hmsc_cbw[ptr->ip][side].bcbwcb_length.BIT.bcbwcb_length = 10;
        break;
    ...
    {
        /* 12bytes */
        g_usb_hmsc_cbw[ptr->ip][side].bcbwcb_length.BIT.bcbwcb_length = USB_MSC_CBWCB_LENGTH;
    }
}
/******************************************************************************
...
void usb_hmsc_set_els_cbw (usb_utr_t *ptr, uint8_t *cbwcb, uint32_t trans_byte, uint16_t side)
{
    ...
    switch (cbwcb[0])
    {
        /* No data */
        case USB_ATAPI_TEST_UNIT_READY :
            g_usb_hmsc_cbw[ptr->ip][side].bcbwcb_length.BIT.bcbwcb_length = 6;
        break;
...
static uint16_t usb_hmsc_get_string_desc (usb_utr_t *ptr, uint16_t addr, uint16_t string, usb_cb_t complete)
{
    ...
    if (USB_OK == err)
    {
        hmsc_retval = usb_hmsc_req_trans_wait_tmo((uint16_t)3000);
...
#else /* (BSP_CFG_RTOS_USED) */
    return (usb_hstd_transfer_start_req(&g_usb_hmsc_class_control[ptr->ip]));
#endif /* (BSP_CFG_RTOS_USED) */
}
/******************************************************************************
    ...
    while (ofdsc < (length - table[0]))
    {
        if (USB_DT_ENDPOINT == table[ofdsc + 1]) /* Search within Interface */
        {
            if (USB_EP_BULK == (table[ofdsc + 3] & USB_EP_TRNSMASK)) /* Bulk Endpoint */
            {
                ...
                usb_hmsc_smp_drive2_addr (side, &devadr);           /* Get USB Device address -> "devadr.keyword" */

                if (USB_EP_IN == (pdescriptor[2] & USB_EP_DIRMASK))
                {
                    g_usb_hmsc_in_pipe[ip][side] = usb_hstd_make_pipe_reg_info (ip, devadr.keyword, USB_HMSC, speed,
                                                             pdescriptor, &g_usb_hmsc_pipe_table[side][USB_PIPE_DIR_IN]);
                }
                else
                {
                    g_usb_hmsc_out_pipe[ip][side] = usb_hstd_make_pipe_reg_info (ip, devadr.keyword, USB_HMSC, speed,
                                                             pdescriptor, &g_usb_hmsc_pipe_table[side][USB_PIPE_DIR_OUT]);
                }
...
uint16_t usb_hmsc_get_string_info(usb_utr_t *mess, uint16_t addr, uint16_t string)
{
    ...
    /* Send GetDescriptor(String) */
    retval = usb_hmsc_get_string_desc(mess, addr, (uint16_t)string, usb_hmsc_req_trans_result);
    if (USB_OK != retval)
    {
        return USB_ERROR;
    }
...
void usb_hmsc_driver_start (uint16_t ip_no)
{
    ...
    for (i = 0; i < USB_MAXSTRAGE; i++)
    {
        ...
        g_usb_hmsc_cbw[ip_no][i].bm_cbw_flags.BIT.cbw_dir = 0;
        g_usb_hmsc_cbw[ip_no][i].bm_cbw_flags.BIT.reserved7 = 0;
        g_usb_hmsc_cbw[ip_no][i].bcbw_lun.BIT.bcbw_lun = 0;
        g_usb_hmsc_cbw[ip_no][i].bcbw_lun.BIT.reserved4 = 0;
        g_usb_hmsc_cbw[ip_no][i].bcbwcb_length.BIT.bcbwcb_length = 0;
        g_usb_hmsc_cbw[ip_no][i].bcbwcb_length.BIT.reserved3 = 0;
