 ...
 ******************************************************************************/
 ...
 #if defined(USB_CFG_HHID_USE)
  #define UX_PORT_HC_AVAILABLE_BANDWIDTH    (2322UL)
 #else
  #define UX_PORT_HC_AVAILABLE_BANDWIDTH    (2304UL)
 #endif                                /* defined(USB_CFG_HHID_USE) */
 #define UX_PORT_MAX_BULK_PAYLOAD           (512UL)
 #define UX_PORT_MAX_INTERRUPT_PAYLOAD      (512UL)
 #define UX_PORT_MAX_CONTROL_PAYLOAD        (512UL)
 #define UX_PORT_MAX_ISO_PAYLOAD            (512UL)

 #define UX_PORT_MAX_SELF_POWER             (500U / 2)
 #define UX_PORT_MASK_ENDPOINT_TYPE         (3U)
 #define UX_PORT_CONTROL_ENDPOINT           (0U)
 #define UX_PORT_SET_CONFIGURATION          (9U)
 #define UX_PORT_SETUP_REQUEST_TYPE         (0U)
 #define UX_PORT_BULK_ENDPOINT              (2U)
 #define UX_PORT_INTERRUPT_ENDPOINT         (3U)
 #define UX_PORT_ISOCHRONOUS_ENDPOINT       (1U)
...
static usb_err_t    usb_peri_usbx_initialize_common(uint32_t dcd_io);
...
usb_err_t   usb_peri_usbx_pmsc_media_initialize(void const * p_context);
void        usb_peri_usbx_set_control_length(usb_setup_t * p_req);

bool        g_usb_peri_usbx_is_detach[USB_MAX_PIPE_NO + 1];

  #if defined(USB_CFG_PMSC_USE)
usb_err_t   usb_peri_usbx_media_initialize(void const * p_context);
usb_err_t   usb_peri_usbx_media_close(void);
 ...
 * Return          : USB_SUCCESS / USB_ERR_NG
 ******************************************************************************/
usb_err_t usb_peri_usbx_initialize (uint32_t dcd_io)
{
    usb_err_t status;
 ...
 * Return          : USB_SUCCESS
 ******************************************************************************/
usb_err_t usb_peri_usbx_uninitialize (uint32_t dcd_io)
{
    UX_SLAVE_DCD * dcd;
...
static void usb_peri_usbx_transfer_complete_cb (usb_utr_t * p_mess, uint16_t data1, uint16_t data2)
{
    ...
    hw_usb_write_pipesel(USB_NULL, pipe);
    pipe_reg = hw_usb_read_pipecfg(USB_NULL);
 ...
 *                 : UINT function      :
 *                 : VOID *parameter    :
 * Return          : UX_SUCCESS / UX_TRANSFER_ERROR / UX_CONTROLLER_UNKNOWN
 ******************************************************************************/
static UINT usb_peri_usbx_to_basic (UX_SLAVE_DCD * dcd, UINT function, VOID * parameter)
{
    ...
    switch (function)
    {
        ...
        case (uint32_t) UX_DCD_TRANSFER_REQUEST:
        {
            ...
            if (0 != endpoint_index)
            {
                pipe = usb_pstd_epadr2pipe((uint16_t) endpoint_index);
            }
            else
            {
                pipe = USB_PIPE0;
            ...
            else
            {
                if (transfer_request->ux_slave_transfer_request_phase == UX_TRANSFER_PHASE_DATA_OUT)
                {
                    if (USB_PERI == g_usb_usbmode)
                    {
                        ...
                        usb_pstd_ctrl_read(size, transfer_request->ux_slave_transfer_request_data_pointer);
                    }
                }
                else
                {
                    ...
                    {
                        {
                            ...
                            g_usb_pstd_std_request = USB_YES;
                            usb_pstd_ctrl_write(size, transfer_request->ux_slave_transfer_request_data_pointer);
                        }
                    }
                }
            }
...
static UINT usb_host_usbx_initialize_common (UX_HCD * hcd)
{
    ...
    hcd->ux_hcd_available_bandwidth = UX_PORT_HC_AVAILABLE_BANDWIDTH;
 ...
 * Return          : UX_SUCCESS
 ******************************************************************************/
usb_err_t usb_host_usbx_uninitialize (uint32_t hcd_io)
{
    UX_HCD * hcd;
...
void usb_host_usbx_attach_init (uint8_t module_number)
{
    ...
    device->ux_device_max_power     = UX_PORT_MAX_SELF_POWER;
    ...
    tx_semaphore_create(&g_usb_host_usbx_sem[module_number][USB_PIPE0], "USB_USBX_SEM_HOST", 0);
...
static UINT usb_host_usbx_to_basic (UX_HCD * hcd, UINT function, VOID * parameter)
{
    ...
    switch (function)
    {
        case (uint32_t) UX_HCD_TRANSFER_REQUEST:
        {
            ...
            switch ((endpoint->ux_endpoint_descriptor.bmAttributes) & (uint32_t) UX_PORT_MASK_ENDPOINT_TYPE)
            {
                case (uint32_t) UX_PORT_CONTROL_ENDPOINT:
                {
  #if defined(USB_CFG_HHID_USE)
                    if (0x0 == (transfer_request->ux_transfer_request_type & UX_REQUEST_TYPE_CLASS))
                    {
                        ...
                        if (UX_PORT_SET_CONFIGURATION == transfer_request->ux_transfer_request_function)
                        {
                        ...
                        {
                            g_usb_host_usbx_request[module_number][UX_PORT_SETUP_REQUEST_TYPE] =
                ...
                case (uint32_t) UX_PORT_BULK_ENDPOINT:
                case (uint32_t) UX_PORT_INTERRUPT_ENDPOINT:
                {
  #if defined(USB_CFG_HMSC_USE)
                ...
                case (uint32_t) UX_PORT_ISOCHRONOUS_ENDPOINT:
                default:
                {
                    break;
                }
        ...
        case (uint32_t) UX_HCD_CREATE_ENDPOINT:
        {
            ...
            switch ((((UX_ENDPOINT *) parameter)->ux_endpoint_descriptor.bmAttributes) & UX_PORT_MASK_ENDPOINT_TYPE)
            {
                case (uint32_t) UX_PORT_CONTROL_ENDPOINT:
                {
                    ((UX_ENDPOINT *) parameter)->ux_endpoint_transfer_request.ux_transfer_request_maximum_length =
                        UX_PORT_MAX_CONTROL_PAYLOAD;
                    break;
                }
