...
#if ((BSP_CFG_RTOS_USED == 5) && defined(USB_CFG_HHID_USE))
/* Enumeration Table */
...
#else   /* (BSP_CFG_RTOS_USED == 5) && defined(USB_CFG_HHID_USE) */
/* Enumeration Table */
...
#endif  /* (BSP_CFG_RTOS_USED == 1) && defined(USB_CFG_HHID_USE) */
...
static void usb_hstd_mgr_rel_mpl (usb_utr_t *ptr, uint16_t n)
{
    ...
    ret = rtos_release_fixed_memory(&g_rtos_usb_mpf_id, (void *)p_usb_shstd_mgr_msg[ptr->ip]);
    if (RTOS_SUCCESS != ret)
    {
                    ...
                    for (flg = 0u, md = 0; (md < g_usb_hstd_device_num[ptr->ip]) && (0u == flg); md++)
                    {
                        ...
                        if (USB_DETACHED == driver->devstate)
                        {
                            ...
                            usbx_status = _ux_host_stack_interfaces_scan(g_p_usbx_configuration,
                                                                (uint8_t *) g_usb_hstd_config_descriptor[ptr->ip]);
 #endif                              /* BSP_CFG_RTOS_USED == 5 */
#else /* (BSP_CFG_RTOS_USED != 0) */
...
if (caseeeee == track_id){
    	    printf("Get discriptor conf - 0x40\n");
}
        break;
        case 4 :
            ...
            usb_shstd_std_req_msg[ptr->ip].p_tranadr = g_usb_hstd_config_descriptor[ptr->ip];
if (caseeeee == track_id){
    	    printf("Get discriptor conf - table = 0x%x\n", usb_shstd_std_request[ptr->ip][3]);
}
        break;
        default :
            return;
...
void usb_hstd_mgr_task (rtos_task_arg_t stacd)
{
    ...
    /* Receive message */
    ret = rtos_receive_mailbox (&g_rtos_usb_mgr_mbx_id, (void **)&mess, (rtos_time_t)10000);
    if (RTOS_SUCCESS != ret)
    {
